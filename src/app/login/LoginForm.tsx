        ...
        <div className="space-y-6 text-center">
          ...
          <div className="flex flex-col gap-4 mt-6">
            <button
              ...
              className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-(--color-accent) hover:text-white hover:border-(--color-accent) transition-all"
            >
              12さい以下
            </button>
            <button
      ...
      {step === 'GITHUB_LOGIN' && (
        <div className="space-y-6 text-center">
          ...
          <button
            ...
            disabled={loading}
            className="w-full p-4 mt-4 flex items-center justify-center gap-3 bg-(--color-accent) text-white rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 font-bold"
          >
      ...
      {step === 'SECRET_SPELL' && (
        <div className="space-y-6 text-center">
          <h2 className="text-2xl font-bold flex items-center justify-center gap-2">
            <Wand2 className="w-6 h-6 text-(--color-accent)" />
            ひみつのじゅもん
          </h2>
          ...
          <form onSubmit={handleSecretSpellLogin} className="space-y-4">
            <input
              ...
              className="w-full p-4 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:outline-none focus:border-(--color-accent)"
            />
            <button
              type="submit"
              disabled={loading || !spell}
              className="w-full p-4 flex items-center justify-center gap-3 bg-(--color-accent) text-white rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 font-bold"
            >
